...
def profile_update():
    ...
    if request.method == 'POST':
        ...
        gender=request.form.get('gender')
        date_of_birth=request.form.get('date_of_birth')
        print(name, email, password, phone, gender, date_of_birth)

        # Update user details
        conn = get_db_connection()
        cursor = conn.cursor()

        cursor.execute('UPDATE Users SET name = ?, email = ?, phone=?, gender=?, date_of_birth=? WHERE user_id = ?', (name, email, phone, gender, date_of_birth, user_id))
        if password:  
            hashed_password = generate_password_hash(password)
...
def parental_monitoring():
    ...
    try:
        ...
        cursor.execute('''
            SELECT task_name, due_date, status, completion_date
            FROM Tasks
            WHERE user_id = ?
        ''', (user_id,))
        ...
        # Fetch user's goals
        cursor.execute('''
            SELECT goal_description, target_date, status
            FROM Goals
            WHERE user_id = ?
        ''', (user_id,))
        ...
        return render_template('parental_monitoring.html', tasks=tasks, goals=goals)
    except:
        pass
...
def enroll(course_name):
    ...
    ''', (session['user_id'], c['content_id'], course_name, 0, 0, 0, None, datetime.now()))
       ...
       # flash(f'Successfully enrolled in {course_name}!', 'success')
        return render_template('course_enroll.html', course_name=course_name, content_id=content_id,start_date=c['start_date'])
    else:
